...
export default function SubmitNSFWReviewPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setMessage({ type: "success", text: "NSFW review submitted! It will appear after admin approval." });
      setDisplayName("");
      setRating(5);
      setReviewText("");
      setImageFile(null);
  ...
  return (
    <div className="rounded-2xl border border-red-500/20 bg-red-500/5 p-6 md:p-8">
      <h3 className="text-xl md:text-2xl font-bold text-white font-display heading-pop mb-2">Submit NSFW Review</h3>
      <p className="text-sm text-gray-400 mb-6">Share your experience with adult VRChat avatar work. This review will only be visible on the NSFW page.</p>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div>
          <label className="block text-sm font-medium text-gray-300 mb-2">Commission Image/Video (optional)</label>
          <div
            ...
            onDrop={handleDrop}
            onClick={() => document.getElementById("nsfw-review-image-upload")?.click()}
          >
            <input id="nsfw-review-image-upload" type="file" accept="image/*,video/*" className="hidden" onChange={(e) => e.target.files?.[0] && handleImageChange(e.target.files[0])} />
        ...
        <button type="submit" disabled={submitting} className="w-full rounded-lg bg-brand-purple-600 px-6 py-3.5 text-sm font-semibold text-white hover:bg-brand-purple-500 disabled:opacity-50 transition-colors btn-glow">
          {submitting ? "Submitting..." : "Submit NSFW Review"}
        </button>
      </form>
    </div>
  );
