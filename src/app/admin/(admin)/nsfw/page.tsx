...
function EmptyState({ onUpload }: { onUpload: () => void }) {
  return (
    <div className="text-center py-20 rounded-xl border border-dashed border-white/10">
      ...
      <p className="text-gray-400 mb-4 text-sm">No NSFW work yet.</p>
...
export default function NSFWPortfolioPage() {
  ...
  const handleFiles = useCallback(async (files: FileList) => {
    ...
    for (const item of queue) {
      try {
        const uploadUrlRes = await fetch("/api/admin/nsfw/upload-url", {
          method: "POST",
        ...
        console.error("NSFW upload error:", message);
  ...
  async function handleToggleVisibility(id: string) {
    ...
    if (!item) return;
    const res = await fetch(`/api/admin/nsfw/${id}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-white font-display">NSFW Portfolio</h1>
          <p className="text-gray-400 mt-1 text-sm">18+ Adult content. Manage separately from SFW work.</p>
        </div>
        <span className="inline-flex items-center gap-1.5 rounded-full border border-red-500/30 bg-red-500/5 px-3 py-1 text-xs font-medium text-red-400">
      ...
      >
        ...
        <p className="text-white font-medium mb-1">DROP YOUR NSFW ARTWORK HERE</p>
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="rounded-xl border border-white/5 bg-white/[0.02] p-6 space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-lg font-semibold text-white font-display">{editingId ? "Edit Work" : "Add NSFW Work"}</h3>
